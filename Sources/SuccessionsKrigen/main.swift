...
func testSDL() {
    ...
    /* Starting SDL */
    func panic(reason: String) -> Never {
        ...
        fatalError("\(reason), error message: \(errorMessage) (code: \(SDL_GetError()!.pointee))")
    }
    ...
    //        return rwFromMem
    //    }
    //    guard let bmp = SDL_LoadBMP_RW(rw, 1 /* `1` indicates that the stream will be closed after read */) else {
    //        panic(reason: "SDL_LoadBMP_RW failed")
    //    }
    guard let imageSurface = IMG_Load("/Users/sajjon/Developer/Fun/Games/HoMM/SuccessionsKrigen/Sources/SuccessionsKrigen/homm2_battle.png") else {
        panic(reason: "`IMG_Load` failed")
    }
    print("Successfully loaded imageSurface 🚀, height: \(imageSurface.pointee.h), width: \(imageSurface.pointee.w)")
